...
class audioPlayer(object):
    ...
    def _getInput(self, channel, audioToPlay, useVolume, normalizeTestSignal):
        ...
        d, c =signal.butter(4,self._hpf/22050,btype='high')
        out_audio= signal.filtfilt(d,c,out_audio)
        f, e =signal.butter(2,self._lpf/22050,btype='low')
        out_audio= signal.filtfilt(f,e,out_audio)
        if (channel==0):
    ...
    def measureChannel(self, channel=0, audioToPlay=None, useVolume=True, normalizeTestSignal=False,hpf=1,lpf=20000):
        ...
        counter = 0
        #HIER FILTER
        #d, c =signal.butter(2,1000/22050,btype='high')
        #output= signal.filtfilt(d,c,output)
